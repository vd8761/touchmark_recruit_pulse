...
function wrapEmailTemplate(content: string, previewText: string = "FoundersEdge Update") {
  return `
    <!DOCTYPE html>
    <html>
    <head>
...
export async function sendUserInviteEmail(email: string, name: string, tempPassword: string) {
  ...
  try {
    await resend.emails.send({
      from: FROM_EMAIL,
      to: email,
      subject: "Welcome to FoundersEdge",
      html: wrapEmailTemplate(`
        <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; color: #333;">
          <h2 style="color: #0f172a;">Welcome, ${name}!</h2>
          <p>An administrator has created an account for you on the FoundersEdge portal.</p>
...
export async function sendClientCreatedAlert(clientName: string, companyName: string, creatorName: string = "System") {
  ...
  await resend.emails.send({
    ...
    html: wrapEmailTemplate(`
      ...
      <p>A new client has been added to FoundersEdge.</p>
      ${buildKeyValueTable([
...
export async function sendDailySummaryAlert(summaryData: { clientName: string; openPositions: number; closedPositions: number; activeResources: number; fulfilledResources: number }[]) {
  ...
  await resend.emails.send({
    ...
    html: wrapEmailTemplate(`
      ...
      <p style="margin-top: 30px; font-size: 12px; color: #64748b;">This is an automated report generated by FoundersEdge.</p>
    `),
  });
}
